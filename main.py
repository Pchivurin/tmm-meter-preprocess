...
def crop_enhance(img, x1, y1, x2, y2, scale, clip):
    h, w = img.shape[:2]
    # розширення вгору (Locate зсуває вниз) + вшир
    hgt, wdt = y2 - y1, x2 - x1
    ex1 = max(0, x1 - wdt * 0.15 - 20)
    ex2 = min(1000, x2 + wdt * 0.25 + 30)
    ey1 = max(0, y1 - hgt * 1.5 - 40)
    ey2 = min(1000, y2 + hgt * 0.2)
...
READ_SYS = (
    ...
    "4. {ANCHOR}"
    "5. Справжня кома відділяє останні 1-2 цифри. Читай ПОВНІСТЮ З КОМОЮ (напр. 048457.99).\n"
    "6. Якщо нечітко — confidence=low.\n"
...
@app.get("/")
def health():
    return {"status": "ok", "service": "meter-ocr", "version": "3.0", "has_key": bool(ANTHROPIC_KEY)}
