...
@app.get("/")
def health():
    return {"status": "ok", "service": "meter-ocr", "version": "3.4", "has_key": bool(ANTHROPIC_KEY)}
...
def _build_buh(rows, period):
    """Бухгалтерія: квартири/офіси/паркінги/оренда, цілі показники."""
    from openpyxl import Workbook
    from openpyxl.styles import Font, Alignment, Border, Side, PatternFill
    import io as _io
    thin = Border(*[Side(style="thin")] * 4)
    wb = Workbook(); ws = wb.active; ws.title = "Квартиры и офисы"
    ws["A1"] = f"Олимпийская э.счетчики {period}"
    ws["A1"].font = Font(name="Arial", bold=True, size=11)
    hdr = ["номер кв", "Серійний номер", "Попередні покази", "Покази", "Різниця розрахована"]
    ws.append(hdr)
    for c in range(1, len(hdr) + 1):
        cell = ws.cell(row=2, column=c)
        cell.font = Font(name="Arial", bold=True, size=10)
        cell.fill = PatternFill("solid", start_color="D9E1F2")
        cell.border = thin; cell.alignment = Alignment(wrap_text=True, horizontal="center")
    srt = sorted(rows, key=lambda r: _apt_sort_key(r.apartment))
    for r in srt:
        ws.append([r.apartment, r.serial, _intval(r.prev_reading), _intval(r.reading_int), _intval(r.diff)])
    for row in ws.iter_rows(min_row=3, max_row=2 + len(srt), min_col=1, max_col=len(hdr)):
        for cell in row:
...
def _build_obl(rows, object_name, period):
    """Обленерго-ЗАГОТОВКА (спрощена, у стилі еталона): щитові, з шапкою й підписами.
    Заповнюємо що маємо (№, лічильник, призначення, покази, різниця, коеф., спожито=різниця×коеф.).
    Договір/рахунок/реактивну — Енергетик дозаповнює вручну."""
    from openpyxl import Workbook
    from openpyxl.styles import Font, Alignment, Border, Side, PatternFill
    import io as _io
    thin = Border(*[Side(style="thin")] * 4)
    wb = Workbook(); ws = wb.active; ws.title = "Отчет"
    ws["A1"] = "Звіт про показання лічильників за розрахунковий місяць"
    ws["A2"] = f"{period}"
    ws["A3"] = f"м. Харків, вул. {object_name}"
    ws["A4"] = 'Фірма "Т.М.М." - ТОВ'
    for r in (1, 2, 3, 4):
        ws.cell(r, 1).font = Font(name="Arial", bold=(r == 1), size=11 if r == 1 else 10)
    hdr = ["№ з/п", "Номер договору", "Особовий рахунок", "Номер лічильника",
           "Покази поточні", "Покази попередні", "Різниця", "Коеф.",
           "Спожито кВт*год", "Приміщення"]
    hr = 6
    ws.append([])  # row5 порожній
    ws.append(hdr)
    for c in range(1, len(hdr) + 1):
        cell = ws.cell(row=hr, column=c)
        cell.font = Font(name="Arial", bold=True, size=9)
        cell.fill = PatternFill("solid", start_color="D9E1F2")
        cell.border = thin; cell.alignment = Alignment(wrap_text=True, horizontal="center", vertical="center")
    n = 0
    for r in rows:
        n += 1
        koef = _intval(r.koef) or 1
        diff = _intval(r.diff)
        spozh = (diff * koef) if isinstance(diff, int) else ""
        ws.append([n, "", "", r.serial, _intval(r.reading_int), _intval(r.prev_reading),
                   diff, koef, spozh, r.naznachennya])
    last = hr + n
    for row in ws.iter_rows(min_row=hr + 1, max_row=last, min_col=1, max_col=len(hdr)):
        for cell in row:
            cell.border = thin; cell.font = Font(name="Arial", size=9)
    # підписи
    sr = last + 2
    ws.cell(sr, 1, "управління нерухомістю                                        Є.В.Травкін")
    ws.cell(sr + 2, 1, "Погоджено:")
    ws.cell(sr + 4, 1, "Виконавець:")
    for i, w in enumerate([6, 14, 14, 16, 14, 14, 10, 8, 14, 30], start=1):
        ws.column_dimensions[chr(64 + i)].width = w
    buf = _io.BytesIO(); wb.save(buf)
    return base64.b64encode(buf.getvalue()).decode()
...
def export(req: ExportRequest):
    # поділ за КАТЕГОРІЄЮ: Щитовая → Обленерго; решта → бухгалтерія
    buh_rows = [r for r in req.rows if str(r.category).strip().lower() != "щитовая"]
    obl_rows = [r for r in req.rows if str(r.category).strip().lower() == "щитовая"]
    buh = _build_buh(buh_rows, req.period)
    obl = _build_obl(obl_rows, req.object_name, req.period)
    return {"buhgalteria_b64": buh, "oblenergo_b64": obl,
            "count": len(req.rows), "buh_count": len(buh_rows), "obl_count": len(obl_rows)}
